...
export function* fetchTodos() {
  const response = yield axios.get(`${getBaseUrl()}/api/todos`);
...
export function* destroyAllTodos() {
  const filtered = yield select(state => state.todos.filter(todo => todo.completed));
...
export function* completeAllTodos() {
  ...
  const allAreMarked = todos.every(todo => todo.completed);
  const newTodos = todos.map(todo => ({ ...todo, completed: !allAreMarked }));
